...
switch($accion){
    case"Agregar":
    $sentenciaSQL= $conexion->prepare("INSERT INTO entrenamientos (nombre, archivo) VALUES (:nombre,:archivo);");
    ...
    break;
     ...
     case"Modificar":
        ...
        $sentenciaSQL= $conexion->prepare("UPDATE  entrenamientos SET nombre=:nombre WHERE id=:id");
        $sentenciaSQL->bindParam(':nombre',$txtNombre);
       ...
       if($txtArchivo!=""){
        ...
        $sentenciaSQL= $conexion->prepare("SELECT archivo FROM entrenamientos WHERE id=:id");
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute();
        $entrenamiento=$sentenciaSQL->fetch(PDO::FETCH_LAZY);

        if( isset($entrenamiento["archivo"]) &&($entrenamiento["archivo"]!="archivo.pdf") ){
        ...
        $sentenciaSQL= $conexion->prepare("UPDATE  entrenamientos SET archivo=:archivo WHERE id=:id");
        $sentenciaSQL->bindParam(':archivo',$nombreDocumento);
        $sentenciaSQL->bindParam(':id',$txtID);
        $sentenciaSQL->execute();
        ...
        case"Cancelar":
          header ("Location:Entrenamiento.php");
            ...
            case"Seleccionar":
                ...
                $sentenciaSQL= $conexion->prepare("SELECT * FROM entrenamientos WHERE id=:id");
                $sentenciaSQL->bindParam(':id',$txtID);
                $sentenciaSQL->execute();
                $entrenamiento=$sentenciaSQL->fetch(PDO::FETCH_LAZY);
                
           $txtNombre=$entrenamiento['nombre'];
           $txtArchivo=$entrenamiento['archivo'];
           ...
           //echo "presionado boton Seleccionarr";
                ...
                case"Borrar":
                  ...
                  $sentenciaSQL= $conexion->prepare("DELETE  FROM entrenamientos WHERE id=:id");
                    $sentenciaSQL->bindParam(':id',$txtID);
                    $sentenciaSQL->execute();
...
$sentenciaSQL= $conexion->prepare("SELECT * FROM entrenamientos");
$sentenciaSQL->execute();
...
<table class="table table-bordered">
    ...
    <tr>
            <td><?php echo $entrenamiento['id']; ?></td>
            <td><?php echo $entrenamiento['nombre']; ?></td>
            <td>

            <img  class ="img-thumbnail rounded" src="../../img/<?php echo $entrenamiento['archivo']; ?>" width="50" alt="" srcset="">
        ...
        <td>
           <form method="post">
            ...
            <input type="hidden" name="txtID" id="txtID"value="<?php echo $entrenamiento['id']; ?>" />
